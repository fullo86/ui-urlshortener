...
async function getAnalytic(code) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/analytics/${code}`,
    {
      method: "GET",
    }
  );
    ...
    <div className="flex flex-col p-4">
      ...
      <div className="flex flex-col gap-4 mt-8">
        <div className="flex">
          ...
          <Link
            href={res.data.url.original_url}
            className="text-blue-300"
            target="_blank"
          >
            {res.data.url.original_url}
          </Link>
        </div>
        ...
        <div className="flex">
          ...
          <Link
            href={res.data.url.short_url}
            className="text-blue-300"
            target="_blank"
            prefetch={false}
          >
            {res.data.url.short_url}
          </Link>
        </div>
      </div>
